...
class PredNet(Recurrent):
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
                 ...
                 data_format='channels_first', iterations = 8, compressor_model=None, **kwargs):
        ...
        self.compressor_model = 'C:\\Users\\meais\\JNotebooks\\Video_Compression\\prednet\\model_data_keras2\\youtube\\encoder_model.json'
        self.compressor_weights = 'C:\\Users\\meais\\JNotebooks\\Video_Compression\\prednet\\model_data_keras2\\youtube\\encoder_weights.hdf5'
    ...
    def get_initial_state(self, x):
        input_shape = self.input_spec[0].shape        # (nt,3,h,w)
        
        init_nb_row = input_shape[self.row_axis]      # height
        init_nb_col = input_shape[self.column_axis]   # width
    ...
    def build(self, input_shape):
        ...
        if self.compressor_model is not None:
            i_s = (input_shape[-3], input_shape[-2], input_shape[-1])
            self.compressor = Compressor(model= self.compressor_model, weights= self.compressor_weights, input_shape=i_s, iterations = self.iterations)
        else:
        ...
        for l in range(self.nb_layers):
            for c in ['i', 'f', 'c', 'o']:  # input, forget, output gate and cell/memory
        ...
        #for input no of channels
        for c in sorted(self.conv_layers.keys()):
            for l in range(len(self.conv_layers[c])):
                ds_factor = 2 ** l
                if c == 'ahat':
                    nb_channels = self.R_stack_sizes[l]     #ahat = relu(conv(R_curr_layer))
                elif c == 'a':
                    nb_channels = 2 * self.stack_sizes[l]   #a= maxpool(conv(E)) and E is +ve + -ve . Therefore, 2 *
                else:
        ...
        self.states = [None] * self.nb_layers*3  #[None] is the batch size || 3 = maybe u r maintaining states of 3 variables

        if self.compressor is not None:
            # self.t_extrap = K.variable(self.extrap_start_time, int if K.backend() != 'tensorflow' else 'int32')
            self.states += [None] * 1  # [timestep]


    def step(self, a, states):
        
        r_tm1 = states[:self.nb_layers]     # r_tminus1
        ...
        for l in reversed(range(self.nb_layers)):
            ...
            c.insert(0, _c)     #insert at first position of the list
            r.insert(0, _r)
        ...
        for l in range(self.nb_layers):
            ...
            if l == 0:
                ...
                if t!=0:
                    error_image = a - ahat
                    recon_error_image = self.compressor.encode(error_image)
                    a = ahat + recon_error_image
    # upto this point is enough for prediction. After this, all error calculation is done with orig: a
    
            # compute errors
            e_up = self.error_activation(ahat - a)  #for l=0, a is the orig image that was passed as a param to step (or the states[-2] in case of extra_time_p)
