...
def resume(epoch=None):
    ...
    encoder.load_state_dict(
        torch.load('checkpoint/encoder_m4_{}_{:08d}.pth'.format(s, epoch)))
    binarizer.load_state_dict(
        torch.load('checkpoint/binarizer_m4_{}_{:08d}.pth'.format(s, epoch)))
    decoder.load_state_dict(
        torch.load('checkpoint/decoder_m4_{}_{:08d}.pth'.format(s, epoch)))
...
def save(index, epoch=True):
    ...
    torch.save(encoder.state_dict(), 'checkpoint/encoder_m4_{}_{:08d}.pth'.format(
        s, index))

    torch.save(binarizer.state_dict(),
               'checkpoint/binarizer_m4_{}_{:08d}.pth'.format(s, index))

    torch.save(decoder.state_dict(), 'checkpoint/decoder_m4_{}_{:08d}.pth'.format(
        s, index))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--iterations', '-i', type=int, default=8, help='unroll iterations')
